...
export class DealerCompany {
  ...
  @Column({ name: 'tier_name', length: 64, default: '二级特约经销商' }) tierName!: string
  @Column({ name: 'discount_rate', type: 'decimal', precision: 4, scale: 2, default: 0.75 }) discountRate!: number
  @Column({ name: 'contact_name', length: 64 }) contactName!: string
  @Column({ name: 'contact_phone', length: 32 }) contactPhone!: string
  @Column({ name: 'contact_email', length: 128 }) contactEmail!: string
