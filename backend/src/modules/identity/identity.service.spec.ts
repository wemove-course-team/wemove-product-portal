...
describe('IdentityService', () => {
  ...
  it('registers a USER with a bcrypt password hash', async () => {
    const users = repository(); const resets = repository(); const service = new IdentityService(users as any, resets as any, { signAsync: jest.fn() } as any, config as any, {} as any)
  ...
  it('rejects a duplicate username or email', async () => {
    const users = repository([{ id: 1, username: 'demo_user', email: 'demo@example.com' }]); const service = new IdentityService(users as any, repository() as any, {} as any, config as any, {} as any)
  ...
  it('rejects invalid credentials and disabled users', async () => {
    const users = repository([{ id: 1, username: 'demo_user', email: 'demo@example.com', passwordHash: '$2b$10$invalid', status: 0 }]); const service = new IdentityService(users as any, repository() as any, {} as any, config as any, {} as any)
  ...
  it('requires an active account when reading the current user', async () => {
    const users = repository([{ id: 1, status: 0 }]); const service = new IdentityService(users as any, repository() as any, {} as any, config as any, {} as any)
    await expect(service.getById('1')).rejects.toMatchObject({ status: 401 })
  })

  it('limits user listing to safe pagination values', async () => {
    const users = repository([{ id: 1, username: 'a', email: 'a@example.com', status: 1 }]); const service = new IdentityService(users as any, repository() as any, {} as any, config as any, {} as any)
