<template>
  <section class="dealer-admin">
    <div class="page-head"><div><h1>经销商申请审核</h1><p>查看申请、保存审核备注，并决定是否授予经销商身份。</p></div><el-button :loading="loading" @click="load">刷新</el-button></div>
    <el-alert v-if="error" :title="error" type="error" show-icon class="mb" />
    <el-table v-loading="loading" :data="items" border stripe>
      <el-table-column prop="id" label="申请编号" width="165" />
      ...
      <el-table-column prop="createdAt" label="提交时间" width="175" />
      <el-table-column label="操作" width="150" fixed="right"><template #default="{ row }"><el-button link type="primary" :disabled="row.status !== 'PENDING'" @click="openReview(row)">审核</el-button></template></el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogVisible" title="审核经销商申请" width="520px">
      <el-descriptions v-if="selected" :column="1" border>
        <el-descriptions-item label="申请编号">{{ selected.id }}</el-descriptions-item>
        <el-descriptions-item label="企业名称">{{ selected.companyName }}</el-descriptions-item>
        <el-descriptions-item label="联系人">{{ selected.contactName }} / {{ selected.phone }}</el-descriptions-item>
        <el-descriptions-item label="目标区域">{{ selected.region }}</el-descriptions-item>
      </el-descriptions>
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top" class="review-form">
        <el-form-item label="审核结果" prop="action"><el-radio-group v-model="form.action"><el-radio label="APPROVED">通过</el-radio><el-radio label="REJECTED">拒绝</el-radio></el-radio-group></el-form-item>
        <el-form-item v-if="form.action === 'APPROVED'" label="经销商等级"><el-input v-model="form.tierName" /></el-form-item>
        <el-form-item v-if="form.action === 'APPROVED'" label="折扣率"><el-input-number v-model="form.discountRate" :min="0.01" :max="1" :step="0.01" /></el-form-item>
        <el-form-item label="审核备注" prop="auditNote"><el-input v-model="form.auditNote" type="textarea" :rows="3" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="dialogVisible = false">取消</el-button><el-button type="primary" :loading="reviewing" @click="submitReview">保存审核结果</el-button></template>
    </el-dialog>
  </section>
</template>
...
const dealerStore = useDealerStore(); const items = ref([]); const total = ref(0); const page = ref(1); const pageSize = ref(10); const loading = ref(false); const reviewing = ref(false); const error = ref(''); const dialogVisible = ref(false); const selected = ref(null); const formRef = ref(null)
...
<style scoped>
.dealer-admin { background: #fff; border: 1px solid var(--border-color); border-radius: 12px; padding: 24px; box-shadow: var(--shadow-sm); }
...
@media (max-width: 768px) { .dealer-admin { padding: 16px; overflow: hidden; } .page-head { align-items: flex-start; } }
</style>
